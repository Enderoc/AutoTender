...
class App(tk.Tk):
    ...
    # ---------------------------------------------------------
    # Безопасное получение фокуса
    # ---------------------------------------------------------
    def safe_focus_get(self):
        """Безопасное получение виджета в фокусе без KeyError"""
        try:
    ...
    # ---------------------------------------------------------
    # Горячие клавиши (ИСПРАВЛЕННЫЕ)
    # ---------------------------------------------------------
    def _bind_hotkeys(self):
        # Не блокируем стандартные комбинации полностью
        # Просто добавляем свою обработку поверх
    ...
    def _on_ctrl_keycode(self, event):
        """ИСПРАВЛЕННЫЙ метод обработки горячих клавиш"""
        ...
        # Безопасное получение виджета в фокусе
        widget = self.safe_focus_get()

        if widget is None:
            return "break"

        # Проверяем тип виджета безопасно
        try:
        ...
        if event.keycode in keycode_commands:
            ...
            try:
                ...
                logging.debug(f"Ошибка при обработке команды {cmd}: {e}")
    ...
    # ---------------------------------------------------------
    # Запуск в потоке (УЛУЧШЕННЫЙ)
    # ---------------------------------------------------------
    def _run_in_thread(self, job, on_done=None, progressbar=None, buttons_to_disable=None):
        """Запускает долгую операцию в отдельном потоке с обновлением интерфейса"""
        ...
        # Периодически обновляем интерфейс пока поток работает
        def keep_alive():
            if thread.is_alive():
                try:
                    self.update_idletasks()
